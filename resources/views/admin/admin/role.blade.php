...
@section('body')
    <div style="margin-top: 10px;width: 100%;margin-left: 10px;">
        <button class="layui-btn layui-btn-normal layui-btn-radius" onclick="layer_show('添加角色','{{url('admin/admin_role_add')}}')">添加角色</button>
    </div>
...
@section('script')
    <script>
        ...
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('admin/admin_role_list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'ID', minWidth:80, sort: true, fixed: 'left'}
                    ,{field: 'name', title: '角色', minWidth:80, fixed: 'left'}
                    // ,{field: 'role_name', title: '角色', minWidth:80, fixed: 'left'}
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    ...
                    layer_show('编辑','{{url('admin/admin_role_add')}}?id='+data.id);
                } else if(obj.event === 'permission'){
                    layer_show('修改权限','{{url('admin/permission')}}?id=' + data.id);
                }
            });
