...
@section('body')
    <div style="margin: 10px;">
        <div style="margin-top: 10px;width: 100%;margin-left: 10px;">
            <button class="layui-btn layui-btn-normal layui-btn-radius" onclick="layer_show('添加分类','{{url('admin/news_category_add')}}')">添加分类</button>
        </div>
...
@section('script')
    <script>
        ...
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('admin/news_category_list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'ID', minWidth:80, align: 'center', sort: true, fixed: 'left'}
                    ,{field: 'name', title: '分类名称', minWidth:80, align: 'center', fixed: 'left'}
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'{{url('admin/news_category_del')}}',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
                    ...
                    layer_show('编辑','{{url('admin/news_category_add')}}?id='+data.id);
                }
            });
